...
simple scenarios, 50 lines of each in batch_0.csv:
(1) measured MAP remains about targetMAP, for which PID xy
should succeed.
(2) measured MAP starts 16 mmHg below target, and PID should "fail" to reach targetMAP,
Each line has 12 MAP measurements, and these are used (each) to train the NN-PID.
...
See Driver_NN_PID_setup, which creates batch_0 as well as a fluid losses file.
...
dbg8 = False # stop at NN data splits
...
numRows = 20   # i-loop will run batch_n.through this many changes to run thru PID
numParms = 3      # number of PID parameters to be optimized
batchSize = 20    # NN's batches (may be different from baseBatch size M.
                   # and even fileSize, which is the size currently read by NN.
...
M = 1     # multiplier of basic time steps, 10 over 1 second
T = 10    # time constant in the step response of "proportional"
TI = M * 10   # total simulation time
...
def NeuralNet (filename):
    """
    Modfied from NN_for_PID in the module test.
    The NN in turn, is based on
    https://gist.github.com/NiharG15/cd8272c9639941cf8f481a7c4478d525
    (See SimpleNN for correct original.)
    NN has 10 inputs, 3 output PID parameters and accuracy and loss
    """
    data = []
    fileSize = 0
    if(dbg3):
    ...
    try:
        ...
        fileSize = count
        if (fileSize != M):
            print("*** Beginning of NN, fileSize = ",fileSize)
            print(print_frame())
    except OSError as e_in:
        print("OS error: {0}".format(err),", output ",new_batch," couldn't be openned")
        print(print_frame())
    except ValueError:
        print("Could not convert data to an integer.")
        print(print_frame())
        ...
        print("Unexpected error at 364:", sys.exc_info()[0])
        print(print_frame())
        raise
    finally:
        inp.close()
        if (dbg1): 
            print("-NN input file, size = ",fileSize)
            print("rows[1] ",rows[1])
            print("Opened filename ",filename," normally.")
            print(" ")
    if(dbg8):
        set_trace() #=============================================
    dataset = array(data)
    xy = dataset.shape
    if (preview): print("data.shape = ",xy)

    # separate the data in two, currently about 100 entries
    split = 10

    train, test = dataset[:split,:], dataset[split:,:]
    
    if(preview):
        print("NN split dataset: train----------------")
        print(train)
        print("NN split dataset: test-----------------")
        print(test)
        print("================================")
        print(" ")

    train_x = train[:,0:12]
    train_y = train[:,-1]   # includes main inputs to PID
    ...
    # This seems best at 11 input nodes, and shape 11, and 1 output node
    if(dbg3):
        set_trace()  ###########################
    model.add(Dense(12, input_shape=(12,), activation='relu', name='fc1'))
    ...
    # Must use sparse_categorical_crossentropy because scores and accurat are floats between 0 and 1
    ...
    # Train the model.  
    # Epochs could be much more than 20, but we want to get an assessment initially of this 
    # architecture so let's not run a long time.
    if(dbg3):
        set_trace()  ###########################
    model.fit(train_x, train_y, verbose=2, batch_size=batchSize, epochs=20)
...
def derivative(h1, i):
    ...
    if ((i < T - 1) and (i > 0) and (i <= len_h) and (len_h > 1)):
    ...
    def advance_PID(self):
        ...
        try:
            ...
            # subsequent time steps
                ...
                if(dbg1): print("-advance_PID returning error ",e1[-11:-1])
...
if (training):
    ...
    After the first loop there are predicted steps in best directions of each of
    the 3 PID parameters individually as the Inner-loop runs 0,..,2.
    This  is a crude way of estimating a gradient.
    In the i-loop, the three steps are added to the parameters, in K,
    in a new baseBatch for the next itereation.
    ...
    # Read entire training file, selecting batches
    try:
    ...
    # Note: instead of this loop, we could run through a list of "baseBatches".
    for n1 in range(0, training_cycles - 1):   #  Run several training sets through PID-NN
        ...
        if(dbg1):
            if (count != M):
                ...
                print(print_frame())
                break
        # write baseBatch for the record, to be read later and varied as deltaBatches
...
#=========================================================
    ...
    ####fileRoot_test = "test_batch_"  # could be random impulses for example
    ...
    baseBatch = fileRoot_test+str(i)+".csv"
    ...
    try:
        ...
        print("OS error at 1158: {0}".format(err))
        stop = True
    except ValueError:
        print("Could not convert data to an integer.")
        stop = True
